...
const { bookmarks, isLoading, error, totalCount, currentPage, pageLimit } =
...
<template>
  <AppLayout>
    ...
    <div class="mt-6">
      ...
      <div
        v-else-if="error"
        class="bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300 p-4 rounded-md"
      >
        {{ error }}
      </div>
      ...
      <div v-else-if="bookmarks.length === 0" class="text-center py-12">
        <p class="text-gray-600 dark:text-gray-400 text-lg">
          {{ t('bookmarks.no_bookmarks_found') }}
        </p>
        <p class="text-gray-500 dark:text-gray-500 text-sm mt-2">
          {{ t('bookmarks.create_first_bookmark') }}
        </p>
      </div>
            ...
            <div class="flex-1 min-w-0">
              <div class="flex justify-between items-start">
                <div class="flex items-start gap-2 flex-1 min-w-0">
                  <h3
                    class="text-blue-600 dark:text-blue-400 font-medium truncate"
                  >
                ...
                <div class="flex space-x-2 ml-4 flex-shrink-0">
                  ...
                  >
                    <span class="sr-only">
                      {{ t('bookmarks.edit_bookmark_action') }}
                    </span>
                  ...
                  >
                    <span class="sr-only">
                      {{ t('bookmarks.delete_bookmark_action') }}
                    </span>
              ...
              <div
                class="text-gray-500 dark:text-gray-400 text-sm mt-1 truncate"
              >
                {{ bookmark.url }}
              </div>
              <div
                v-if="bookmark.excerpt && !shouldHideExcerpt"
    ...
    <DeleteConfirmationModal
      :is-open="bookmarkToDelete !== null"
      :title="t('bookmarks.delete_bookmark')"
      :message="t('bookmarks.confirm_delete_message')"
      :item-name="bookmarkToDelete?.title || bookmarkToDelete?.url"
      :is-loading="isDeleting"
      @close="cancelDeleteBookmark"
