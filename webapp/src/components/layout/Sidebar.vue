...
const adminSection: NavSection = {
    ...
    subsections: [
        ...
        { nameKey: 'admin.user_management', icon: UsersIcon, route: '/admin/users' }
    ]
};
...
const isSectionActive = (section: NavSection) => {
    ...
    if (section.subsections) {
        return section.subsections.some(subsection => isRouteActive(subsection.route));
    }
    return false;
};
    ...
    <div>
        <template v-if="!isMobile">
            ...
            <aside
                ...
                <nav class="flex flex-col space-y-1 flex-1 px-3">
                    ...
                    <div v-for="section in navSections" :key="section.nameKey" class="space-y-1">
                        <!-- Section Header -->
                        <div v-if="section.subsections" class="space-y-1">
                            <RouterLink :to="section.route"
                                class="flex items-center space-x-3 px-3 py-2 rounded-md transition-colors"
                                :class="isSectionActive(section) ? 'text-red-500 bg-red-50 dark:bg-red-900/20 dark:text-red-400' : 'text-gray-600 dark:text-gray-400 hover:text-red-500 hover:bg-gray-100 dark:hover:bg-gray-700'">
                                <component :is="section.icon" class="h-5 w-5 flex-shrink-0" />
                                <span class="text-sm font-medium dark:text-gray-300">{{ t(section.nameKey) }}</span>
                            </RouterLink>

                            <!-- Subsections - Always visible on desktop -->
                            <div class="ml-6 space-y-1">
                                <RouterLink v-for="subsection in section.subsections" :key="subsection.nameKey"
                                    :to="subsection.route" :class="[
                                    ...
                                    <component :is="subsection.icon" class="h-4 w-4 flex-shrink-0" />
                                    <span class="text-xs font-medium dark:text-gray-400">{{ t(subsection.nameKey)
                                    }}</span>
                                </RouterLink>
                            </div>
                        </div>
                        ...
                        <RouterLink v-else :to="section.route" :class="[
                            'px-3 py-2 rounded-md transition-colors flex items-center space-x-3',
                    ...
                    <div v-if="authStore.user?.owner" class="space-y-1">
                        <RouterLink :to="adminSection.route"
                            class="flex items-center space-x-3 px-3 py-2 rounded-md transition-colors"
                            :class="isSectionActive(adminSection) ? 'text-red-500 bg-red-50 dark:bg-red-900/20 dark:text-red-400' : 'text-gray-600 dark:text-gray-400 hover:text-red-500 hover:bg-gray-100 dark:hover:bg-gray-700'">
                            <component :is="adminSection.icon" class="h-5 w-5 flex-shrink-0" />
                            <span class="text-sm font-medium dark:text-gray-300">{{ t(adminSection.nameKey) }}</span>
                        </RouterLink>

                        <!-- Admin Subsections - Always visible on desktop -->
                        <div class="ml-6 space-y-1">
                            <RouterLink v-for="subsection in adminSection.subsections" :key="subsection.nameKey"
                                :to="subsection.route" :class="[
                                    'px-3 py-2 rounded-md transition-colors flex items-center space-x-3',
                                    isRouteActive(subsection.route)
                                        ? 'text-red-500 bg-red-50 dark:bg-red-900/20 dark:text-red-400'
                                ...
                                <component :is="subsection.icon" class="h-4 w-4 flex-shrink-0" />
                                <span class="text-xs font-medium dark:text-gray-400">{{ t(subsection.nameKey) }}</span>
                            </RouterLink>
                        </div>
                    </div>
                    ...
                    <div class="relative mt-auto px-3" ref="menuRef">
                        <button @click.stop="toggleMenu"
                            ...
                            <span class="text-sm font-medium dark:text-gray-300">{{ authStore.user?.username ||
                                t('auth.user') }}</span>
                        </button>
        ...
        <template v-else>
            ...
            <nav
                ...
                <RouterLink v-for="section in navSections" :key="section.nameKey" :to="section.route"
                    ...
                    <span class="text-xs mt-1 dark:text-gray-300">{{ t(section.nameKey) }}</span>
                </RouterLink>
                <RouterLink v-if="authStore.user?.owner" :to="adminSection.route"
                    class="text-gray-500 dark:text-gray-400 hover:text-red-500 p-2 flex flex-col items-center">
                    <component :is="adminSection.icon" class="h-6 w-6" />
                    <span class="text-xs mt-1 dark:text-gray-300">{{ t(adminSection.nameKey) }}</span>
                </RouterLink>
            </nav>
        </template>
    </div>
